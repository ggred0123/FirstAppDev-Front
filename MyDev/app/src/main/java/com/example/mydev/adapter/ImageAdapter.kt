...
class ImageAdapter(private val context: Context, private var images: List<String>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        // Glide를 사용해 URL에서 이미지 로드
        Glide.with(context)
            .load(images[position]) // URL 리스트에서 이미지 로드
            .placeholder(R.drawable.ic_placeholder) // 로딩 중 보여줄 기본 이미지
            .error(R.drawable.ic_error) // 오류 시 보여줄 기본 이미지
            .into(imageView)

        return imageView
    }

    // 새로운 이미지 리스트로 갱신하는 메서드 추가
    fun updateImages(newImages: List<String>) {
        images = newImages
        notifyDataSetChanged() // 데이터 변경 후 UI 업데이트
    }
}
